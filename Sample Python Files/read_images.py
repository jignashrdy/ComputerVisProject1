...
# read gray level images

# lenna.bmp is gray image
# fruits.jpg is color image
# without flag it is not clear, depends on the default
# with IMREAD_GRAYSCALE it is converted to gray scale (even if it is not)
gray_image1 = cv2.imread('pic.jpg') # mistake?
gray_image2 = cv2.imread('pic.jpg', cv2.IMREAD_GRAYSCALE)
gray_image3 = cv2.imread('pic.jpg', cv2.IMREAD_UNCHANGED)
...
cv2.namedWindow('color4', cv2.WINDOW_AUTOSIZE)
RGB_image1 = cv2.cvtColor(BGR_image1, cv2.COLOR_BGR2RGB)
cv2.imshow('color4', RGB_image1) # mistake ?
